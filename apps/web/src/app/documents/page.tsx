...
const[cid,scid]=useState(null);const[docs,sd]=useState([]);const[ld,sld]=useState(true);
...
async function loadD(id){sld(true);try{const sn=await getDocs(query(collection(db,'documents'),where('clubId','==',id),orderBy('createdAt','desc')));sd(sn.docs.map(d=>({id:d.id,...d.data()})));}catch(e){}finally{sld(false);}}
const hl=async()=>{await signOut(auth);window.location.href='/login';};const hc=id=>{scid(id);loadD(id);};
...
{ld?<div className="space-y-3">{[1,2].map(i=><div key={i} className="card"><div className="skeleton h-6 w-48"/></div>)}</div>:docs.length===0?<div className="empty-state"><div className="empty-state-icon">📄</div><p className="empty-state-title">Knowledge base is empty</p><p className="empty-state-text">Upload club documents to build your knowledge base. Supported formats: PDF, DOCX, TXT</p></div>:<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">{docs.map(d=><div key={d.id} className="card"><h3 className="font-semibold">{d.name}</h3><p className="text-xs text-gray-400 mt-2">{(d.sizeBytes/1024).toFixed(0)} KB · {new Date(d.createdAt).toLocaleDateString()}</p></div>)}</div>}
</div></main></div>);}